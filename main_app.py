...
from setting import PACKAGE, mobile_3, DEV, server_addr
...
def init_dev(uuids):
    ...
    auto_setup(
        __file__, 
        devices=[f"Android://{server_addr[0]}:{server_addr[1]}/{uuid}?cap_method=JAVACAP&&ori_method=MINICAPORI&&touch_method=MINITOUCH" for uuid in uuids], 
        # logdir=True, 
        compress=90)
...
def _verify(poco):
    ...
    verify = 0
    while verify < 2:
        if poco(text="继续安装").wait(8).exists():
    ...
    if poco(nameMatches="^首页\n.+").exists() and poco("第 1 个标签，共 3 个").exists():
        ...
        if poco("欢迎登录油葱商城").exists():
            ...
            if poco("请完成安全验证").wait(2).exists():
                poco("请完成安全验证").child("android.view.View")[1].focus([0.17, 0.55]).wait(8).swipe([0.68, 0.002], duration=0.5)
                time.sleep(0.2)
                
            poco(text="请输入短信验证码").wait(8).click()
...
def init_all(uuid, mobile):
    ...
    dev =connect_device(f'Android://{server_addr[0]}:{server_addr[1]}/{uuid}?cap_method=MINICAP&&ori_method=MINICAPORI&&touch_method=MINITOUCH')
